...
struct EmailView: View {
    ...
    private var markdown: some View {
        ...
        var index = 0
        attributedString.enumerateAttribute(NSAttributedString.Key.imgSource, in: NSRange(0..<attributedString.length), using: { value, range, _ in
        ...
        return VStack {
            ForEach(blocks) { block in
                switch block {
                case let .image(source, _, _): KFImage(URL(string: source ?? "")).resizable().aspectRatio(contentMode: .fit)
